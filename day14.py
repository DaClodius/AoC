def part_one(value):
    memory[0][index] = int(''.join([bitmask[pointer] if bitmask[pointer].isnumeric() else value[pointer] for pointer in range(len(bitmask))]), 2)

def part_two(index):
    index = [bitmask[pointer] if bitmask[pointer] == '1' else index[pointer] for pointer in range(len(index))]
    for numeric in range(pow(2, bitmask.count('X'))):
        for binary_index, binary_digit in enumerate(binary_string(numeric, bitmask.count('X'))):
            index[len(bitmask) - len(bitmask.split('X', binary_index + 1)[-1]) - 1] = binary_digit
            memory[1][int(''.join(index), 2)] = value
